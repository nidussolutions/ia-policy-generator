...
router.post('/:id/prices', adminAuthMiddleware, async (req: AdminAuthRequest, res: Response):Promise<any> => {
  ...
  try {
    ...
    // Update the plan in local database with the new price
    const updatedPlan = await prisma.plans.update({
      where: { id },
      data: {
        price: newPrice.id,
...
router.post('/sync', adminAuthMiddleware, async (req: AdminAuthRequest, res: Response) => {
  try {
    ...
    for (const product of stripeProducts) {
      ...
      for (const price of productPrices) {
        ...
        if (localPlanId) {
          ...
          // Create new plan for active prices not in local database
    ...
    // Find plans in local database that no longer exist in Stripe
